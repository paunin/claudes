...
config_dir()  { print -r -- "$HOME/.claude-$1"; }
data_dir()    { print -r -- "$HOME/Library/Application Support/Claude-${INSTANCE_LABEL[$1]}"; }
bundle_id()   { print "com.anthropic.claude.$1"; }
...
make_icns() {
  ...
  swift "$HERE/render-icon.swift" "$work/icon.iconset" "${INSTANCE_LABEL[$slug]}" "$hue"
